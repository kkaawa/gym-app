...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  select.innerHTML = syumokuList.map(s =>
...
function updateChart() {
  ...
  // 選択した種目だけ絞り込む
  let filtered = allRecords.filter(r => r.syumoku === syumoku);
  ...
  const labels  = filtered.map(r => r.date);   // X軸：日付
  const weights = filtered.map(r => r.omosa);  // Y軸：重量
